...
from pandas_vet import options


def test_set_format():
    options.set_format(precision=3, use_emojis=False)
    assert pd.get_option("vet.precision") == 3
    assert pd.get_option("vet.use_emojis") == False
...
def test_reset_format():
    ...
    options.reset_format()
    assert pd.get_option("vet.precision") == 2
    assert pd.get_option("vet.use_emojis") == True


def test_set_mode():
    options.set_mode(enable_checks=False, enable_asserts=False)
    assert pd.get_option("vet.enable_checks") == False
    assert pd.get_option("vet.enable_asserts") == False
...
def test_enable_checks():
    ...
    assert pd.get_option("vet.enable_checks") == True
    assert pd.get_option("vet.enable_asserts") == True
...
def test_register_option_valid():
    ...
    assert pd.get_option("vet.test_option") == 10
...
def test_register_option_existing():
    ...
    assert pd.get_option("vet.precision") == 10
